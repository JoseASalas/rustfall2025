...
fn sharing_resource_refcell_count() {
    ...
    let mut me = FamilyMember { tv: Rc::clone(&dad.tv) };
    let mut remote_control = me.tv.borrow_mut();
    println!("TV channel {:?}", remote_control);

    remote_control.channel = "CNN".to_string();
    println!("TV channel {:?}", remote_control);
    drop(remote_control);
    println!("TV channel for me {:?}", mom.tv);
    println!("People are watching {}", Rc::strong_count(&me.tv));
